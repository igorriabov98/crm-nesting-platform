...
import type { CurrentUser, UserDepartmentMembershipSummary } from '@/lib/types'
...
function getPositions(user: CurrentUser) {
  const positions = new Map<string, UserDepartmentMembershipSummary['position']>()
  for (const membership of user.department_memberships || []) {
    if (membership.position) positions.set(membership.position.id, membership.position)
  }
  return Array.from(positions.values()).filter(Boolean)
}

export function UserTable({ users, currentUser, canManage }: UserTableProps) {
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const matchesSearch =
      ...
      (u.department_memberships || []).some((membership) =>
        membership.department?.name.toLowerCase().includes(query) ||
        membership.position?.name.toLowerCase().includes(query)
      )
  ...
  const toggleBanStatus = async (user: CurrentUser) => {
    ...
    if (user.id === currentUser.id) {
      toast.error('Нельзя заблокировать собственный аккаунт')
      return
    }
        ...
        <Table>
          <TableHeader className="bg-[#F8F9FA]">
            <TableRow className="border-[#E8ECF0] hover:bg-transparent">
              ...
              <TableHead className="text-[#6B7280]">Отделы</TableHead>
              <TableHead className="text-[#6B7280]">Должности</TableHead>
              <TableHead className="text-[#6B7280]">Начальник</TableHead>
          ...
          <TableBody>
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((u) => {
                ...
                const isDepartmentHead = memberships.some((membership) => membership.is_department_head)
                ...
                return (
                  <TableRow key={u.id} className="border-[#E8ECF0] hover:bg-[#F8F9FA]">
                    ...
                    <TableCell className="max-w-[260px] text-sm text-[#374151]">
                    ...
                    <TableCell className="max-w-[220px] text-sm text-[#374151]">
                      {positions.length > 0 ? positions.map((position) => position?.name).join(', ') : '—'}
                    </TableCell>
                    <TableCell>
                      {isDepartmentHead ? (
                        <Badge variant="outline" className="text-amber-600 border-amber-400/30 bg-amber-400/10">
                          Да
                        </Badge>
                      ) : (
                        <span className="text-sm text-[#9CA3AF]">Нет</span>
                      )}
                    </TableCell>
                    <TableCell>
                      {u.is_active ? (
