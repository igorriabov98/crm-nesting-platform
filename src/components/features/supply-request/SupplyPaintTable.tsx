...
export function SupplyPaintTable({ rows, machineId, canReserve = false, canUnreserve = false }: Props) {
  return (
              ...
              <tr key={row.id}>
                ...
                <td className={tdClass}>
                  {(canReserve || (canUnreserve && row.reservation_id)) ? <div className="flex items-center gap-2">
                    {canReserve && <ReserveButton table="request_paint" itemId={row.id} materialId={row.material_id} machineId={machineId} needed={needed} reserved={reserved} covered={row.covered_quantity} available={row.available_stock} unit={unit} stockItems={row.stock_items} />}
                    {canUnreserve && row.reservation_id && <UnreserveButton table="request_paint" itemId={row.id} />}
