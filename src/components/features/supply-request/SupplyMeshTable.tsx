...
import type { RequestMesh } from '@/lib/types'
...
export function SupplyMeshTable({ rows, machineId, canReserve = false, canUnreserve = false }: Props) {
  return (
    <Section title="Сетка">
      <table className={tableClass}>
        <thead className="border-b border-[#E8ECF0] bg-[#F8F9FA]">
          <tr>
            {['Материал', 'Характеристика решетки', 'Длина, мм', 'Ширина, мм', 'Необходимо, шт', 'На складе', 'Забронировано', 'Статус', 'Действия'].map((header, index) => (
              <th key={header} className={`${thClass} ${index === 0 ? stickyCellClass : ''}`}>{header}</th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-[#F1F5F9]">
          {rows.length === 0 ? <EmptyRows colSpan={9} /> : rows.map((row) => {
              ...
              <tr key={row.id}>
                ...
                <td className={tdClass}>
                  {(canReserve || (canUnreserve && row.reservation_id)) ? <div className="flex items-center gap-2">
                    {canReserve && <ReserveButton table="request_mesh" itemId={row.id} materialId={row.material_id} machineId={machineId} needed={needed} reserved={reserved} covered={row.covered_quantity} available={row.available_stock} unit={unit} stockItems={row.stock_items} />}
                    {canUnreserve && row.reservation_id && <UnreserveButton table="request_mesh" itemId={row.id} />}
