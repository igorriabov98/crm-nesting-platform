...
async function RequestResultDetailPage({ params }: { params: Promise<{ requestId: string }> }) {
  ...
  const { request, versions, canReview, canEdit, currentSnapshot, currentDraft, revisionDraft, sheetScrapStates } = result.data
  const order = Array.isArray(request.machines) ? request.machines[0] : request.machines
  const stock = request.request_kind === 'stock'
  const displayNumber = stock ? `СЗ-${String(request.request_number).padStart(6, '0')}` : `№${request.request_number}`
  type VersionView = {
    id: string
  ...
  return <main className="mx-auto max-w-6xl space-y-6 pb-16">
    ...
    <header className="flex flex-col justify-between gap-4 rounded-xl border bg-white p-5 sm:flex-row sm:items-start">
      <div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-semibold">Заявка {displayNumber}</h1>{current && <Badge variant="outline" className={approvalBadgeClass(current.state)}>{stateLabels[current.state] || current.state}</Badge>}{revisionDraft && <Badge variant="outline">Черновик редакции {displayNumber}.{revisionDraft.display_revision_number}</Badge>}</div><p className="mt-1 text-slate-600">{stock ? `${request.title} · На склад` : `${order?.name || 'Без названия'} · ${order?.material_type === 'standard' ? 'Стандартный материал' : order?.material_type === 'non_standard' ? 'Нестандартный материал' : 'Тип не выбран'}`}</p></div>
    </header>
    <Card><CardHeader><CardTitle>{currentDraft ? 'Текущий черновик' : 'Текущая сводка'}{current && !currentDraft ? ` · Версия ${stock ? `${displayNumber}.${current.display_revision_number}` : formatApprovalVersion(current.display_revision_number, request.request_number)}` : ''}</CardTitle></CardHeader><CardContent className="space-y-4">{currentDraft && <p className="text-sm text-slate-500">{stock ? 'Состав показан на текущий момент; при повторной отправке он будет зафиксирован в новой версии.' : 'Состав и брони показаны на текущий момент. Отходность, время, будущая деталировка и будущие листовые остатки — по последней отправке; они обновятся после повторного итогового мастера.'}</p>}{!current && <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><p className="font-medium">Ещё не отправлена на согласование</p><p className="mt-1">Текущий этап: {getRequestStatusLabel(request.status)}.</p><Link className="mt-3 inline-flex underline" href={stock ? `${ROUTES.MATERIAL_REQUESTS}/stock/${request.id}` : `${ROUTES.SALES_PLAN}/${request.machine_id}/request/${request.id}`}>Открыть заявку</Link></div>}{current && <ApprovalSummary snapshot={currentSnapshot || null} sheetScrapStates={sheetScrapStates} approvalState={current.state} />}</CardContent></Card>
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><History className="h-5 w-5" />История версий</CardTitle></CardHeader><CardContent className="space-y-3">
      <ApprovalVersionHistory requestId={requestId} requestNumber={request.request_number} stock={stock} versions={typedVersions} />
    </CardContent></Card>
  </main>
}
