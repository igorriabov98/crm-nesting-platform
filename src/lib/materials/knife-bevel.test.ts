...
import {
  knifeBevelCharacteristicLabel,
  knifeBevelLabel,
  parseKnifeBevelCount,
  requireKnifeBevelCount,
} from './knife-bevel'
...
test('accepts only an explicitly selected knife bevel', () => {
  ...
  assert.equal(knifeBevelCharacteristicLabel(1), '1 скос')
})
...
test('does not default a missing or invalid knife bevel', () => {
  ...
  assert.equal(knifeBevelCharacteristicLabel(null), 'не указан')
