...
export function isBusinessScrapReservationStatus(status: string) {
  return status === 'pending_stock_check'
}

export function isSupplyWarehouseReservationStatus(status: string) {
  return status === 'stock_checked'
}

export function isSupplyRequestReadOnlyStatus(status: string) {
  return status === 'submitted_to_supply' || status === 'completed'
}
