...
BEGIN
  IF to_regclass('public.machine_item_nesting_runs') IS NULL THEN
    missing_tables := array_append(missing_tables, 'public.machine_item_nesting_runs');
  END IF;
  IF to_regclass('public.machines') IS NULL THEN
    missing_tables := array_append(missing_tables, 'public.machines');
  END IF;
  ...
  IF to_regclass('public.users') IS NULL THEN
    missing_tables := array_append(missing_tables, 'public.users');
  END IF;

  IF array_length(missing_tables, 1) IS NOT NULL THEN
    RAISE EXCEPTION 'Missing prerequisite tables for nesting batch migration: %', array_to_string(missing_tables, ', ');
  END IF;
END $$;
