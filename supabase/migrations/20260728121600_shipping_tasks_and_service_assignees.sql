...
BEGIN
  FOR v_machine IN
    SELECT DISTINCT m.id, m.name, m.created_by, m.desired_shipping_date, m.is_archived
    FROM public.machines m
    LEFT JOIN public.production_stages ps ON ps.machine_id = m.id AND ps.stage_type = 'shipping'
    LEFT JOIN public.tasks existing_task ON existing_task.machine_id = m.id
    ...
    WHERE existing_task.id IS NOT NULL
       OR COALESCE(ps.planned_date_end, ps.date_end, m.desired_shipping_date) <= CURRENT_DATE + 7
  LOOP
    SELECT COALESCE(ps.planned_date_end, ps.date_end, v_machine.desired_shipping_date)::date
    INTO v_shipping_date
    FROM (SELECT 1) seed
    LEFT JOIN LATERAL (
      SELECT planned_date_end, date_end
      FROM public.production_stages
      WHERE machine_id = v_machine.id AND stage_type = 'shipping'
    ...
    SELECT EXISTS (
      SELECT 1 FROM public.machine_expenses me
      WHERE me.machine_id = v_machine.id
        AND lower(btrim(COALESCE(me.category, ''))) IN ('транспорт', 'транспортные расходы', 'transport', 'transport_cost')
    ...
    LOOP
      ...
      IF CURRENT_DATE < v_deadline OR (v_task.task_type = 'transport_cost' AND v_has_transport_cost) THEN
        UPDATE public.tasks SET status = 'cancelled', updated_at = now()
        WHERE machine_id = v_machine.id AND task_type = v_task.task_type
          AND status IN ('pending', 'in_progress');
        CONTINUE;
      END IF;
      ...
      SET assigned_to = v_assignee,
          ...
          description = v_task.description || ' для машины ' || COALESCE(v_machine.name, 'Машина') || '. Плановая отгрузка: ' || to_char(v_shipping_date, 'DD.MM.YYYY') || '.',
          start_date = v_deadline, deadline = v_deadline, updated_at = now()
      WHERE machine_id = v_machine.id AND task_type = v_task.task_type
        AND status IN ('pending', 'in_progress');

      IF NOT FOUND AND NOT EXISTS (
        SELECT 1 FROM public.tasks
        WHERE machine_id = v_machine.id AND task_type = v_task.task_type AND status = 'completed'
      ) THEN
        INSERT INTO public.tasks (machine_id, assigned_to, task_type, title, description, status, start_date, deadline)
        VALUES (
          v_machine.id, v_assignee, v_task.task_type,
          v_task.title || ': ' || COALESCE(v_machine.name, 'Машина'),
          v_task.description || ' для машины ' || COALESCE(v_machine.name, 'Машина') || '. Плановая отгрузка: ' || to_char(v_shipping_date, 'DD.MM.YYYY') || '.',
          'pending', v_deadline, v_deadline
        )
        ON CONFLICT (machine_id, assigned_to, task_type)
          WHERE machine_id IS NOT NULL AND status IN ('pending', 'in_progress')
        DO UPDATE SET title = EXCLUDED.title, description = EXCLUDED.description,
          start_date = EXCLUDED.start_date, deadline = EXCLUDED.deadline, updated_at = now();
      END IF;
      v_count := v_count + 1;
    END LOOP;
  END LOOP;
