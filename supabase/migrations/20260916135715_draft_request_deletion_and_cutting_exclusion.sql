-- Draft material requests are editable working data. Allow the authorized
-- server action to remove a plain draft atomically, while preserving any
-- request that has already acquired immutable operational dependencies.
create or replace function public.fn_delete_technologist_request_draft_v1(
  p_request_id uuid,
  p_actor uuid
) returns uuid
language plpgsql
...
begin
  ...
  if not found then
    raise exception 'Черновик заявки не найден';
  end if;

  if p_actor is null or not exists (
    select 1 from public.users app_user
    where app_user.id = p_actor and app_user.is_active
  ) then
  ...
  if not coalesce(v_can_manage, false) then
    raise exception 'Недостаточно прав для удаления черновика';
  end if;
  if v_request.status <> 'draft'::public.request_status then
    raise exception 'Удалить можно только заявку в статусе «Черновик»';
  end if;
  if coalesce(v_request.is_recalculation_staging, false) then
    raise exception 'Служебный черновик перерасчёта удаляется только через связанный процесс';
  end if;
  ...
  if exists (
    ...
    raise exception 'Черновик связан с неизменяемой картой раскроя и не может быть удалён';
  end if;
  if exists (
    select 1 from public.supply_position_revisions revision
    where revision.source_request_id = p_request_id
       or revision.replacement_request_id = p_request_id
  ) then
    raise exception 'Черновик связан с исправлением позиции снабжения и не может быть удалён';
  end if;
  if exists (
    select 1 from public.long_stock_recalculation_replacements replacement
    where replacement.source_request_id = p_request_id
       or replacement.replacement_request_id = p_request_id
  ) then
    raise exception 'Черновик связан с перерасчётом карты раскроя и не может быть удалён';
  end if;
  if exists (
    select 1 from public.department_requests request
    where request.technologist_request_id = p_request_id
  ) then
    raise exception 'Черновик связан с заявкой подразделения и не может быть удалён';
  end if;

  delete from public.technologist_requests request
  where request.id = p_request_id
    and request.status = 'draft'::public.request_status
  returning request.machine_id into v_machine_id;

  if v_machine_id is null then
    raise exception 'Черновик изменился. Обновите страницу';
  end if;
  return v_machine_id;
end;
$$;

revoke all on function public.fn_delete_technologist_request_draft_v1(uuid, uuid)
  from public, anon, authenticated;
grant execute on function public.fn_delete_technologist_request_draft_v1(uuid, uuid)
  to service_role;

-- Drafts never enter the production cutting queue. Replace the transactional
-- body behind the serialized public starter so the database and UI enforce the
-- same request set even when a caller bypasses the page.
...
begin
  ...
  where request.machine_id = p_machine_id
    and request.status not in ('draft'::public.request_status, 'cancelled'::public.request_status)
    and not exists (
