...
BEGIN
  ...
  IF v_category = 'knives' THEN
    ...
    v_total_length := COALESCE(
      ...
      CASE WHEN lower(COALESCE(p_unit, '')) IN ('м', 'm') THEN p_total_quantity * 1000 ELSE p_total_quantity END
    );

    RETURN round((v_total_length * v_variant.width_mm * v_variant.height_mm * v_density)::numeric, 2);
  END IF;
...
CREATE OR REPLACE FUNCTION trg_calc_inventory_weight()
RETURNS trigger AS $$
BEGIN
  NEW.calculated_weight_kg := calc_inventory_weight_kg(
    NEW.material_id,
    NEW.material_variant_id,
    NEW.total_quantity,
    NEW.unit,
    NEW.piece_length_mm,
    NEW.total_secondary_quantity
  );
...
DROP TRIGGER IF EXISTS trg_inventory_weight ON inventory;

CREATE TRIGGER trg_inventory_weight
  BEFORE INSERT OR UPDATE OF material_id, material_variant_id, total_quantity, unit, piece_length_mm, total_secondary_quantity
  ON inventory
  FOR EACH ROW
  EXECUTE FUNCTION trg_calc_inventory_weight();
