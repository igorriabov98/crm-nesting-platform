...
BEGIN
  ...
  IF v_category = 'pipe' THEN
    ...
    v_length := COALESCE(p_piece_length_mm * NULLIF(p_total_secondary_quantity, 0), p_total_quantity);
    ...
    RETURN round((v_cross_section * v_length * v_density * COALESCE(p_total_quantity, 0))::numeric, 2);
  END IF;
...
CREATE TRIGGER trg_inventory_weight
  BEFORE INSERT OR UPDATE OF material_id, material_variant_id, total_quantity, unit, piece_length_mm
  ON inventory
  FOR EACH ROW
