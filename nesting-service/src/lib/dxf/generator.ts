...
function pushLocalPartEntities(lines: string[], prepared: PreparedPart, options: ResolvedDxfOptions): void {
  ...
  for (const segment of prepared.leadSegments) {
    pushEntity(lines, dxfLine(segment.from.x, segment.from.y, segment.to.x, segment.to.y, segment.layer));
  }
...
function createLeadSegments(
  contour: Point2D[],
  part: DxfPartData,
  partIndex: number,
  contourName: string,
  sheet: { width: number; height: number },
  obstacleBoxes: PlacedBox[],
  options: ResolvedDxfOptions
...
function segmentIntersectsBox(start: Point2D, end: Point2D, box: PlacedBox): boolean {
