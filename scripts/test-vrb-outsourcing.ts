...
const productProjectForm = read('src/components/features/products/ProductProjectForm.tsx')
const productProjectDetails = read('src/components/features/products/ProductProjectDetailClient.tsx')
...
assert.match(migration, /IF TG_TABLE_NAME = 'machines' THEN[\s\S]*sync_vrb_mesh_for_machine\(NEW\.id\)/)
...
assert.match(productProjectForm, /Требуется сетка VRB/)
assert.match(productProjectForm, /aria-describedby="project_requires_vrb_mesh_help"/)
assert.match(productProjectDetails, /Сетка VRB/)
assert.match(productActions, /requires_vrb_mesh: project\.requires_vrb_mesh/)
