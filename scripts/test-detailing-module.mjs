...
assert.match(requestPage, /aria-label="Поиск по деталировке"/)
assert.match(requestPage, /max-h-\[520px\][^"']*overflow-y-auto/)
assert.match(requestPage, /setDecision\('declined'\)/)
assert.match(requestPage, /decision === 'declined'[\s\S]*Деталировка не используется/)
