...
assert.ok(
  ...
  'Long-stock cutting plan schema tests only use localhost or 127.0.0.1',
)
...
const testSql = readFileSync(
  path.join(root, 'supabase', 'tests', 'long_stock_cutting_plan_schema_test.sql'),
  'utf8',
)
...
assert.equal(result.status, 0, 'Long-stock cutting plan schema SQL assertions failed')
process.stdout.write(result.stdout || '')
console.log('[long-stock-cutting-plan-schema] all assertions passed')
