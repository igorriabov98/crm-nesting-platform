...
assert.ok(
  ...
  'Long-stock recalculation tests only use localhost or 127.0.0.1',
)
const databaseName = decodeURIComponent(databaseUrl.pathname.slice(1))
assert.ok(databaseName.toLowerCase().includes('test'), 'Test database name must contain "test"')

run(process.execPath, [path.join(root, 'scripts', 'test-supply-receiving-plan-fact.mjs')])
...
const testSql = readFileSync(
  path.join(root, 'supabase', 'tests', 'long_stock_cutting_plan_receipt_recalculation_test.sql'),
  'utf8',
)
...
assert.equal(result.status, 0, 'Long-stock receipt recalculation SQL assertions failed')
process.stdout.write(result.stdout || '')
console.log('[long-stock-receipt-recalculation] all assertions passed')
