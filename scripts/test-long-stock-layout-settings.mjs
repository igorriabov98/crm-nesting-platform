...
assert.ok(
  ...
  'Long-stock settings tests only use localhost or 127.0.0.1',
)
...
const testSql = readFileSync(
  path.join(root, 'supabase', 'tests', 'long_stock_layout_settings_test.sql'),
  'utf8',
)
...
assert.equal(result.status, 0, 'Long-stock layout settings SQL assertions failed')
process.stdout.write(result.stdout || '')
console.log('[long-stock-layout-settings] all assertions passed')
