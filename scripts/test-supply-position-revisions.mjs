...
assert.ok(['localhost', '127.0.0.1'].includes(databaseUrl.hostname), 'Revision tests only use localhost')
...
const result = spawnSync('psql', ['-X', '-v', 'ON_ERROR_STOP=1', '-d', databaseName], {
  ...
  encoding: 'utf8',
  env: postgresEnv,
  input: readFileSync(path.join(root, 'supabase', 'tests', 'supply_position_revisions_test.sql'), 'utf8'),
})
if (result.status !== 0) {
  process.stderr.write(result.stdout || '')
  process.stderr.write(result.stderr || '')
}
assert.equal(result.status, 0, 'Supply position revision SQL assertions failed')
