...
assert(uploadRoute.includes('requireTechnologistRequestAccess(input.requestId'), 'Загрузка из мастера должна проверять доступ к конкретной заявке')
assert(uploadRoute.includes("inventoryOperation: 'manage'"), 'Загрузка из мастера должна повторять проверку управления складом')
assert(!uploadRoute.includes("requirePermission('machine_cutting', 'manage')"), 'Своя заявка технолога не должна требовать общее право производственного раздела')
