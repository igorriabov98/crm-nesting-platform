...
const scheduledEnumMigration = source('supabase/migrations/20260903110000_add_scheduled_payment_terms_type.sql')
const scheduledTermsMigration = source('supabase/migrations/20260903111000_scheduled_payment_terms.sql')
...
assert(scheduledEnumMigration.includes("ADD VALUE IF NOT EXISTS 'scheduled_after_delivery'"), 'Новый enum должен добавляться отдельной миграцией')
assert(scheduledTermsMigration.includes('scheduled_payment_weekdays integer[]'))
assert(scheduledTermsMigration.includes('scheduled_payment_month_days integer[]'))
assert(scheduledTermsMigration.includes('scheduled_payment_weekdays_snapshot integer[]'))
assert(scheduledTermsMigration.includes('scheduled_payment_month_days_snapshot integer[]'))
assert(scheduledTermsMigration.includes('CREATE OR REPLACE FUNCTION public.fn_next_scheduled_payment_date'))
assert(scheduledTermsMigration.includes('CREATE OR REPLACE FUNCTION public.fn_scheduled_payment_overdue_amount'))
assert(scheduledTermsMigration.includes('occurrence < p_as_of'), 'В день обязательства официальной просрочки ещё нет')
assert(scheduledTermsMigration.includes("CURRENT_TIMESTAMP AT TIME ZONE 'Europe/Uzhgorod'"), 'Граница дня должна считаться по часовому поясу CRM')
assert(scheduledTermsMigration.includes('expected_amount := LEAST(p_amount, occurrence_count * p_minimum_amount)'), 'Просрочка должна быть накопительной и ограниченной суммой инвойса')
assert(scheduledTermsMigration.includes('scheduled_payment_minimum_amount_snapshot'), 'Условия должны фиксироваться в снимке инвойса')
assert(scheduledTermsMigration.includes('trg_refresh_invoice_due_date_on_delivery') === false, 'Миграция переиспользует существующий триггер доставки без второго конкурирующего триггера')
assert(scheduledTermsMigration.includes('GRANT SELECT ON public.client_list_summary TO authenticated, service_role'))
assert(!/CREATE\s+POLICY[\s\S]{0,120}(?:invoices|invoice_payments)[\s\S]{0,80}(?:INSERT|UPDATE|DELETE)/iu.test(scheduledTermsMigration), 'Нельзя добавлять публичные финансовые мутации')
...
assert(actions.includes('scheduled_payment_weekdays_snapshot'))
assert(actions.includes('scheduled_payment_minimum_amount_snapshot'))
