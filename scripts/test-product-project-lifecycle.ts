...
const atomicCompletion = source('supabase/migrations/20260920220000_technologist_reservations_projects_password.sql')
assert.match(taskActions, /status === 'in_progress'[\s\S]*product_project_engineering[\s\S]*status: 'engineering'/)
assert.match(taskActions, /crm_complete_product_project_engineering_task/, 'task completion must use the atomic RPC')
assert.match(atomicCompletion, /crm_complete_product_project_engineering_task[\s\S]*status='client_review'/)
assert.match(atomicCompletion, /описание инженера/, 'task completion must require the engineer description')
assert.match(atomicCompletion, /product_project_sales_review/, 'atomic completion must create the sales review task')
