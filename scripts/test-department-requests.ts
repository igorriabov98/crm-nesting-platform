...
assert.equal(isDepartmentRequestTarget('planning'), true)
...
assert.equal(DEPARTMENT_REQUEST_TARGETS.planning.route, '/requests')
...
assert.match(requestPage, /department === 'planning'/)
assert.match(requestPage, /redirect\(requestParams\.size > 0/)
