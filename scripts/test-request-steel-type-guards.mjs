...
assert.ok(
  ...
  'Request steel-type guard tests only use localhost or 127.0.0.1',
)
...
const result = spawnSync('psql', ['-X', '-v', 'ON_ERROR_STOP=1', '-d', databaseName], {
  ...
  env: postgresEnv,
  input: readFileSync(
    path.join(root, 'supabase', 'tests', 'request_steel_type_handoff_guard_test.sql'),
    'utf8',
  ),
})
if (result.status !== 0) {
...
assert.equal(result.status, 0, 'Request steel-type guard SQL assertions failed')
process.stdout.write(result.stdout || '')
console.log('[request-steel-type-guards] all assertions passed')
